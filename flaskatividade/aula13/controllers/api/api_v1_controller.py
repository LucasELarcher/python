...
# 1. RESOLVIDO: Rota corrigida com o objeto do blueprint correto
...
def api_listar_locacoes():
    ...
    return jsonify([
        {
            "id": loc.id,
            # 2. RESOLVIDO: Serialização completa do objeto de locação para JSON
            "cliente_id": loc.cliente_id,
            "cliente_nome": loc.cliente.nome if loc.cliente else "Não informado",
            "veiculo_id": loc.veiculo_id,
            "veiculo_modelo": loc.veiculo.modelo if loc.veiculo else "Não informado",
            "veiculo_placa": loc.veiculo.placa if loc.veiculo else "Não informada",
            "data_inicio": loc.data_inicio.strftime("%Y-%m-%d") if loc.data_inicio else None,
            "data_fim": loc.data_fim.strftime("%Y-%m-%d") if loc.data_fim else None,
...
def api_detalhe_locacao(locacao_id):
    ...
    return jsonify({
        "id": loc.id,
        "cliente": loc.cliente.nome if loc.cliente else "Não informado",
        "veiculo": loc.veiculo.modelo if loc.veiculo else "Não informado",
        "placa": loc.veiculo.placa if loc.veiculo else "Não informada",
        "valor_total": loc.valor_total,
    })
...
def api_detalhe_veiculo(veiculo_id):
    ...
    if not veiculo:
        # 3. RESOLVIDO: Código HTTP 404 (Not Found) adicionado
        return jsonify({"erro": "Veículo não encontrado"}), 404
    return jsonify({
        "id": vehicle_id if 'vehicle_id' in locals() else veiculo.id, # Correção preventiva caso variável mude
        "placa": veiculo.placa,
...
def api_modelos_nhtsa():
    # 4. RESOLVIDO: O request.args.get busca parâmetros passados via Query String na URL (?marca=X)
